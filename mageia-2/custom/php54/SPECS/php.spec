...
%define postgresql_version %(pg_config &>/dev/null && pg_config 2>/dev/null | awk '/^VERSION/ { print $4 }'|| echo 0)

%if %{mgaversion} < 3
%define _httpd_apxs %{_sbindir}/apxs
%define _httpd_modconfdir %{_sysconfdir}/httpd/modules.d/
%define _httpd_moddir %{_libdir}/apache-extramodules
%endif
...
Patch25:	php-5.4.16-external_libzip.diff
...
Patch303:	php-5.4.12-mageia-logo.diff
# apache-mod_php
Patch400:	php-5.4.6-apache-mod_php.diff
...
# mod_php
%patch400 -p1
mv sapi/apache2handler/mod_php5.c sapi/apache2handler/mod_php.c
...
./configure \
    ...
    --with-gd=shared --with-jpeg-dir=%{_prefix} --with-vpx-dir=%{_prefix} --with-png-dir=%{_prefix} --with-zlib-dir=%{_prefix} --with-xpm-dir=%{_prefix}/X11R6 --with-freetype-dir=%{_prefix} --enable-gd-native-ttf --with-t1lib=%{_prefix} \
...
install -m0755 .libs/libphp5.so %{buildroot}%{_httpd_moddir}/%{mod_name}.so

cat > %{buildroot}%{_httpd_modconfdir}/%{load_order}_%{mod_name}.conf <<EOF
%if %{mgaversion} < 3
LoadModule php_module extramodules/%{mod_name}.so
%endif
%if %{mgaversion} >= 3
...
%{_localstatedir}/lib/rpm/filetriggers/httpd-php.script
...
%attr(0771,apache,apache) %dir /var/lib/php-fpm
%attr(0771,apache,apache) %dir /var/log/php-fpm
%attr(0771,apache,apache) %dir /var/run/php-fpm
#/var/run/php-fpm/php-fpm.conf
